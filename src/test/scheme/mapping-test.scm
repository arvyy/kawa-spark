(import (class org.apache.http.client.fluent Request Response Content)
        (class org.apache.http.entity ContentType))

(define (test-req req expected-result)
  (let* ((resp (Request:execute req))
         (content (Response:returnContent resp))
         (response-string (Content:asString content)))
    (test-equal (ContentType:toString (ContentType:withCharset ContentType:TEXT_PLAIN "utf-8")) 
                (ContentType:toString (ContentType:withCharset (Content:getType content) "utf-8")))
